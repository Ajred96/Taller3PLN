...
# 2. Argumentos del Experimento 2 (Batch Size = 16)
training_args = TrainingArguments(
    output_dir="./outputs/modelBetoSarcasmo/results_beto_bs16", # Apunta a la subcarpeta del lote 16
    eval_strategy="epoch",
    save_strategy="epoch",
    learning_rate=1e-5,
    per_device_train_batch_size=16, # <-- Cambiado a 16
    per_device_eval_batch_size=16,  # <-- Cambiado a 16
    num_train_epochs=10,
    weight_decay=0.01,
...
trainer = Trainer(
    ...
    callbacks=[EarlyStoppingCallback(early_stopping_patience=5)] # <-- Paciencia configurada en 5
)
...
if __name__ == "__main__":
    print("\n=== Iniciando el entrenamiento de BETO (Batch Size = 16) ===")
    # trainer.train() 